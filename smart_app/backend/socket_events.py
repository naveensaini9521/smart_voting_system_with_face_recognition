...
def register_socket_events(socketio):
    ...
    def default_error_handler(e):
        ...
        if hasattr(request, "sid") and request.sid in connected_clients:
            try:
                safe_emit(
                    "error",
                    {"message": "An unexpected error occurred", "event": request.event},
                )
    ...
    def handle_connect():
        ...
        try:
            ...
            if voter_id:
                ...
                logger.info(
                    f"Voter {voter_id} connected successfully. SID: {request.sid}"
                )
            ...
            # Handle admin connections
                ...
                logger.info(
                    f"Admin {admin_id} connected successfully. SID: {request.sid}"
                )
    ...
    def handle_disconnect(reason=None):
        ...
        try:
            ...
            if client_data:
                ...
                user_id = (
                    client_data.get("voter_id")
                    or client_data.get("admin_id")
                    or "unknown"
                )
    ...
    def handle_subscribe_elections(data):
        ...
        try:
            ...
            if not client_data or client_data.get("voter_id") != voter_id:
                logger.warning(
                    f"Unauthorized subscription attempt for voter {voter_id}"
                )
            ...
            logger.info(
                f"Client {request.sid} subscribed to elections for voter {voter_id}"
            )
    ...
    def handle_start_voting_session(data):
        ...
        try:
            ...
            if not election_id or not voter_id:
                safe_emit(
                    "voting_session_error",
                    {"message": "Election ID and Voter ID required"},
                )
                return
            ...
            safe_emit(
                "voting_session_error", {"message": "Failed to start voting session"}
            )
    ...
    def send_private_notification(voter_id, notification_data):
        ...
        try:
            safe_emit(
                "private_notification", notification_data, room=f"voter_{voter_id}"
            )
    ...
    def get_connected_users_count():
        ...
        return {
            ...
            "anonymous_connected": len(
                [c for c in connected_clients.values() if c.get("type") == "anonymous"]
            ),
        }
    ...
    socketio.broadcast_election_update = types.MethodType(
        broadcast_election_update, socketio
    )
    socketio.broadcast_voter_update = types.MethodType(broadcast_voter_update, socketio)
    socketio.broadcast_system_update = types.MethodType(
        broadcast_system_update, socketio
    )
    socketio.send_private_notification = types.MethodType(
        send_private_notification, socketio
    )
    socketio.get_connected_users_count = types.MethodType(
        get_connected_users_count, socketio
    )

    logger.info(
        "Socket.IO events registered successfully with production authentication"
    )
