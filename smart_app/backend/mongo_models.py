...
class Voter(MongoBase):
    ...
    def create_voter(cls, data):
        ...
        voter_id = cls.generate_unique_voter_id(
            data["national_id_number"], data["date_of_birth"], data["full_name"]
        )
    ...
    def update_face_verification(
        cls, voter_id, face_encoding_id, quality_score, methods_used
    ):
    ...
    def find_all_with_face_encodings(cls):
        ...
        pipeline = [
            ...
            {"$match": {"face_encodings": {"$ne": []}, "face_verified": True}},
            {
                "$project": {
                    "voter_id": 1,
                    "full_name": 1,
...
class OTP(MongoBase):
    ...
    @classmethod
    def create_otp(
        cls, email=None, phone=None, purpose="verification", expires_in_minutes=10
    ):
        """Create a new OTP"""
...
class FaceEncoding(MongoBase):
    ...
    @classmethod
    def create_encoding(
        cls, voter_id, encoding_data, image_metadata=None, knn_indexed=False
    ):
    ...
    def find_by_voter_id(cls, voter_id):
        ...
        return cls.find_one({"voter_id": voter_id, "is_active": True})
    ...
    def update_knn_status(cls, voter_id, is_indexed=True):
        ...
        try:
            return cls.update_one(
                ...
                {"$set": {"knn_indexed": is_indexed, "updated_at": datetime.utcnow()}},
            )
        except Exception as e:
    ...
    def get_encoding_statistics(cls):
        ...
        try:
            ...
            return {
                ...
                "knn_indexed": knn_indexed,
                "knn_percentage": (
                    (knn_indexed / total_encodings * 100) if total_encodings > 0 else 0
                ),
            }
        except Exception as e:
    ...
    def migrate_to_hybrid_format(cls):
        ...
        try:
            ...
            old_encodings = cls.find_all(
                {"is_active": True, "encoding_data": {"$not": {"$type": "object"}}}
            )
...
class User(MongoBase):
    ...
    def update_last_login(cls, user_id):
        ...
        return cls.update_one({"user_id": user_id}, {"last_login": datetime.utcnow()})
...
class Election(MongoBase):
    ...
    def create_election(cls, data):
        ...
        election_data = {
            ...
            "results_visibility": data.get(
                "results_visibility", "after_end"
            ),  # live, after_end, manual
    ...
    def get_active_elections(cls):
        ...
        return cls.find_all({"status": "active", "is_active": True})
    ...
    def update_election_status(cls, election_id, status):
        ...
        return cls.update_one({"election_id": election_id}, {"status": status})
...
class Candidate(MongoBase):
    ...
    def find_by_election(cls, election_id):
        ...
        return cls.find_all({"election_id": election_id, "is_active": True})
...
class Vote(MongoBase):
    ...
    def find_by_election_and_voter(cls, election_id, voter_id):
        ...
        return cls.find_one(
            {"election_id": election_id, "voter_id": voter_id, "is_verified": True}
        )
...
class Admin(MongoBase):
    ...
    def find_by_username(cls, username):
        ...
        try:
            ...
            if admin:
                logger.info(
                    f"Admin found: {admin.get('username')} - {admin.get('admin_id')}"
                )
            else:
    ...
    def find_by_admin_id(cls, admin_id):
        ...
        try:
            ...
            if admin:
                logger.info(
                    f"Admin found by admin_id: {admin.get('username')} - {admin.get('admin_id')}"
                )
            else:
    ...
    def update_last_login(cls, admin_id):
        ...
        return cls.update_one({"admin_id": admin_id}, {"last_login": datetime.utcnow()})
...
class IDDocument(MongoBase):
    ...
    @classmethod
    def verify_document(
        cls, document_id, verified_by, status="approved", rejection_reason=None
    ):
        """Verify ID document"""
        update_data = {
        ...
        return cls.update_one({"document_id": document_id}, update_data)
...
class AuditLog(MongoBase):
    ...
    @classmethod
    def create_log(
        cls, action, user_id, user_type, details, ip_address=None, user_agent=None
    ):
...
class SystemStats(MongoBase):
    ...
    def update_stats(cls, stats_type, data):
        ...
        return cls.update_one(
            {"stats_type": stats_type}, {"$set": data, "$inc": {"update_count": 1}}
        )
...
class VoterEligibility(MongoBase):
    ...
    def check_eligibility(cls, voter_id, election_id):
        ...
        return cls.find_one(
            {"voter_id": voter_id, "election_id": election_id, "is_eligible": True}
        )
...
class ElectionResult(MongoBase):
    ...
    def find_by_election(cls, election_id):
        ...
        return cls.find_one({"election_id": election_id}, sort=[("calculated_at", -1)])
    ...
    def create_or_update_result(cls, election_id, results_data):
        ...
        if existing:
            return cls.update_one({"_id": existing["_id"]}, result_data)
        else:
...
def create_dashboard_indexes():
    ...
    collections.otps.create_index(
        [("expires_at", 1)], expireAfterSeconds=3600
