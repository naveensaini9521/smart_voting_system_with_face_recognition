...
def _serialize_doc(doc):
    ...
    for k, v in doc.items():
        if k == '_id' and isinstance(v, ObjectId):
            ...
            result[k] = [_serialize_doc(item) if isinstance(item, dict) else item for item in v]
        else:
            result[k] = v
    return result
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            if not admin or not admin.get('is_active', True):
                return jsonify({'success': False, 'message': 'Admin account not found or inactive'}), 401
            request.admin = admin
...
def get_dashboard_stats():
    try:
        ...
        stats = {
            ...
            'upcoming_elections': Election.count({"status": "scheduled", "is_active": True, "voting_start": {"$gt": now}}),
            'total_voters': Voter.count({"is_active": True}),
            'verified_voters': Voter.count({"email_verified": True, "phone_verified": True, "id_verified": True, "face_verified": True, "is_active": True}),
            'total_votes': Vote.count({"is_verified": True}),
            'today_votes': Vote.count({"vote_timestamp": {"$gte": today_start}, "is_verified": True}),
            'recent_votes': Vote.count({"vote_timestamp": {"$gte": week_ago}, "is_verified": True}),
            'pending_verifications': Voter.count({"$or": [{"email_verified": False}, {"phone_verified": False}, {"id_verified": False}, {"face_verified": False}], "is_active": True}),
            'total_candidates': Candidate.count({"is_active": True}),
            'voter_turnout': 0  # will compute
        }
        total_voters = stats['total_voters']
        if total_voters:
            stats['voter_turnout'] = round((stats['total_votes'] / total_voters * 100), 1)
        stats['system_health'] = 'optimal'
        stats['last_updated'] = now.isoformat()
        return jsonify({'success': True, 'stats': stats})
    except Exception as e:
        logger.error(f"Dashboard stats error: {str(e)}")
        return jsonify({'success': False, 'message': 'Failed to load dashboard statistics'}), 500
...
def get_elections():
    try:
        ...
        elections_data = []
        for e in paginated:
            vote_count = Vote.count({"election_id": e['election_id'], "is_verified": True})
            candidate_count = Candidate.count({"election_id": e['election_id'], "is_active": True})
            ...
            elections_data.append({
                ...
                'require_face_verification': e.get('require_face_verification', True),
...
def create_election():
    try:
        ...
        if not all(k in data and data[k] for k in required):
            return jsonify({'success': False, 'message': f'Missing required field: {required}'}), 400

        # Parse dates
        voting_start = _parse_datetime(data['voting_start'])
        voting_end = _parse_datetime(data['voting_end'])
        registration_start = _parse_datetime(data.get('registration_start'), default=voting_start - timedelta(days=7))
        registration_end = _parse_datetime(data.get('registration_end'), default=voting_start)
        ...
        now = datetime.utcnow()
        status = "active" if voting_start <= now <= voting_end else "scheduled" if now < voting_start else "completed"

        # File upload handling (simplified)
        election_logo = election_banner = None
        if request.content_type and request.content_type.startswith('multipart/form-data'):
            if 'election_logo' in request.files and request.files['election_logo'].filename:
                election_logo = f"/uploads/elections/logos/{request.files['election_logo'].filename}"
            if 'election_banner' in request.files and request.files['election_banner'].filename:
        ...
        # Broadcast
        _broadcast_election_update('create' if status != 'active' else 'activate',
                                   {'election_id': election_id, 'title': data['title'], 'status': status},
                                   request.admin['admin_id'])
        log_admin_action(request.admin, "create_election",
                         {"election_id": election_id, "title": data['title'], "status": status}, election_id)

        return jsonify({'success': True, 'message': f'Election created - {status}', 'election_id': election_id, 'election': election, 'status': status})
    except Exception as e:
        logger.error(f"Create election error: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'message': f'Failed to create election: {str(e)}'}), 500
...
def get_election_details(election_id):
    try:
        ...
        vote_results = {r['_id']: r['total_votes'] for r in Vote.get_collection().aggregate(pipeline)}
        ...
        return jsonify({'success': False, 'message': 'Failed to load election details'}), 500
...
def update_election(election_id):
    try:
        ...
        if request.content_type and request.content_type.startswith('multipart/form-data'):
            if 'election_logo' in request.files and request.files['election_logo'].filename:
                data['election_logo'] = f"/uploads/elections/logos/{request.files['election_logo'].filename}"
            if 'election_banner' in request.files and request.files['election_banner'].filename:
                data['election_banner'] = f"/uploads/elections/banners/{request.files['election_banner'].filename}"

        Election.update_one({"election_id": election_id}, data)
        updated = Election.find_by_election_id(election_id)
        _broadcast_election_update('update', {'election_id': election_id, 'title': data.get('title', election['title'])}, request.admin['admin_id'])
        log_admin_action(request.admin, "update_election", {"election_id": election_id, "title": data.get('title', election['title'])}, election_id)
        return jsonify({'success': True, 'message': 'Election updated', 'election': updated})
    except Exception as e:
...
def update_election_status(election_id):
    try:
        ...
        if status not in VALID_ELECTION_STATUSES:
            return jsonify({'success': False, 'message': f'Invalid status. Must be one of: {", ".join(VALID_ELECTION_STATUSES)}'}), 400

        election = Election.find_by_election_id(election_id)
        if not election:
            return jsonify({'success': False, 'message': 'Election not found'}), 404

        Election.update_one({"election_id": election_id}, {"status": status, "updated_at": datetime.utcnow()})
        _broadcast_election_update('status_update', {'election_id': election_id, 'title': election['title'], 'old_status': election.get('status'), 'new_status': status}, request.admin['admin_id'])
        log_admin_action(request.admin, "update_election_status", {"election_id": election_id, "old_status": election.get('status'), "new_status": status}, election_id)
        return jsonify({'success': True, 'message': f'Election status updated to {status}'})
    except Exception as e:
        logger.error(f"Update election status error: {str(e)}")
        return jsonify({'success': False, 'message': 'Failed to update election status'}), 500
...
def delete_election(election_id):
    try:
        ...
        if not election:
            return jsonify({'success': False, 'message': 'Election not found'}), 404
        Election.update_one({"election_id": election_id}, {"is_active": False, "status": "cancelled", "updated_at": datetime.utcnow()})
        _broadcast_election_update('delete', {'election_id': election_id, 'title': election['title']}, request.admin['admin_id'])
        log_admin_action(request.admin, "delete_election", {"election_id": election_id, "title": election['title']}, election_id)
...
def get_election_for_edit(election_id):
    try:
        ...
        return jsonify({'success': False, 'message': f'Failed to load election: {str(e)}'}), 500
...
def get_election_results_admin(election_id):
    try:
        ...
        if election.get('voter_turnout') != voter_turnout:
            Election.update_one({"election_id": election_id}, {"voter_turnout": voter_turnout, "total_voters": total_voters, "total_votes": results['total_votes']})
...
def publish_election_results(election_id):
    try:
        ...
        if not election:
            return jsonify({'success': False, 'message': 'Election not found'}), 404
        Election.update_one({"election_id": election_id}, {
        ...
        _broadcast_election_update('results_published', {
            'election_id': election_id,
            'title': election['title'],
        ...
        log_admin_action(request.admin, "publish_results", {"election_id": election_id, "title": election['title']}, election_id)
        return jsonify({'success': True, 'message': 'Results published', 'broadcast_sent': True})
    except Exception as e:
...
def unpublish_election_results(election_id):
    try:
        ...
        if not election:
            return jsonify({'success': False, 'message': 'Election not found'}), 404
        Election.update_one({"election_id": election_id}, {"results_published": False, "results_unpublished_at": datetime.utcnow(), "results_unpublished_by": request.admin['admin_id']})
        log_admin_action(request.admin, "unpublish_results", {"election_id": election_id, "title": election['title']}, election_id)
...
def get_candidates():
    try:
        ...
        for c in paginated:
            ...
            vote_count = Vote.count({"candidate_id": c['candidate_id'], "is_verified": True})
            candidates_data.append({
                'candidate_id': c['candidate_id'],
        ...
        elections_filter = [{'election_id': e['election_id'], 'title': e['title']} for e in all_elections]

        return jsonify({
            'success': True,
            'candidates': candidates_data,
            'pagination': {'page': page, 'per_page': per_page, 'total': total, 'total_pages': total_pages},
            'filters': {'elections': elections_filter, 'approval_status': approval, 'election_id': election_id}
        })
    except Exception as e:
        logger.error(f"Get candidates error: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'message': f'Failed to load candidates: {str(e)}'}), 500
...
def create_candidate():
    try:
        ...
        if party_logo and party_logo.filename:
            candidate_data['party_symbol'] = f"/uploads/parties/logos/{party_logo.filename}"
        if election_symbol and election_symbol.filename:
            candidate_data['election_symbol'] = f"/uploads/candidates/symbols/{election_symbol.filename}"

        candidate_id = Candidate.create_candidate(candidate_data)
        candidate = Candidate.find_by_candidate_id(candidate_id)

        _broadcast_system_update('candidate_created', {'candidate_id': candidate_id, 'candidate_name': data['full_name'], 'election_id': data['election_id']}, request.admin['admin_id'])
        log_admin_action(request.admin, "create_candidate", {"candidate_id": candidate_id, "candidate_name": data['full_name'], "election_id": data['election_id']}, candidate_id)

        return jsonify({'success': True, 'message': 'Candidate created', 'candidate_id': candidate_id, 'candidate': candidate})
    except Exception as e:
        logger.error(f"Create candidate error: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'message': f'Failed to create candidate: {str(e)}'}), 500
    ...
    try:
        if request.content_type and request.content_type.startswith('multipart/form-data'):
            ...
            if 'photo' in request.files and request.files['photo'].filename:
                data['photo'] = f"/uploads/candidates/photos/{request.files['photo'].filename}"
            if 'party_logo' in request.files and request.files['party_logo'].filename:
                data['party_symbol'] = f"/uploads/parties/logos/{request.files['party_logo'].filename}"
            if 'election_symbol' in request.files and request.files['election_symbol'].filename:
                data['election_symbol'] = f"/uploads/candidates/symbols/{request.files['election_symbol'].filename}"
        else:
            data = request.get_json() or {}
        ...
        _broadcast_system_update('candidate_updated', {'candidate_id': candidate_id, 'candidate_name': data.get('full_name', candidate['full_name'])}, request.admin['admin_id'])
        log_admin_action(request.admin, "update_candidate", {"candidate_id": candidate_id, "candidate_name": data.get('full_name', candidate['full_name'])}, candidate_id)
        return jsonify({'success': True, 'message': 'Candidate updated', 'candidate': updated})
    except Exception as e:
...
def delete_candidate(candidate_id):
    try:
        ...
        if not candidate:
            return jsonify({'success': False, 'message': 'Candidate not found'}), 404
        Candidate.update_one({"candidate_id": candidate_id}, {"is_active": False, "updated_at": datetime.utcnow()})
        _broadcast_system_update('candidate_deleted', {'candidate_id': candidate_id, 'candidate_name': candidate['full_name']}, request.admin['admin_id'])
        log_admin_action(request.admin, "delete_candidate", {"candidate_id": candidate_id, "candidate_name": candidate['full_name']}, candidate_id)
...
def approve_candidate(candidate_id):
    try:
        ...
        if not candidate:
            return jsonify({'success': False, 'message': 'Candidate not found'}), 404
        Candidate.update_one({"candidate_id": candidate_id}, {"is_approved": True, "approved_at": datetime.utcnow(), "approved_by": request.admin['admin_id']})
        _broadcast_system_update('candidate_approved', {'candidate_id': candidate_id, 'candidate_name': candidate['full_name'], 'election_id': candidate.get('election_id')}, request.admin['admin_id'])
        log_admin_action(request.admin, "approve_candidate", {"candidate_id": candidate_id, "candidate_name": candidate['full_name']}, candidate_id)
        return jsonify({'success': True, 'message': 'Candidate approved'})
    except Exception as e:
        logger.error(f"Approve candidate error: {str(e)}")
        return jsonify({'success': False, 'message': 'Failed to approve candidate'}), 500
    ...
    try:
        ...
        if verification != 'all':
            if verification == 'verified':
                query.update({"email_verified": True, "phone_verified": True, "id_verified": True})
            elif verification == 'pending':
                query["$or"] = [{"email_verified": False}, {"phone_verified": False}, {"id_verified": False}]
        ...
        voters = Voter.find_all(query, sort=[("created_at", -1)], skip=skip, limit=per_page)
        ...
        for v in voters:
            ...
            voters_data.append({
                ...
                'is_fully_verified': all([v.get('email_verified', False), v.get('phone_verified', False), v.get('id_verified', False)]),
        ...
        return jsonify({
            ...
            'voters': voters_data,
            'pagination': {'page': page, 'per_page': per_page, 'total': total, 'total_pages': total_pages},
            'filters': {'constituencies': constituencies, 'verification_status': verification, 'constituency': constituency}
        })
    except Exception as e:
        logger.error(f"Get voters error: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'message': f'Failed to load voters: {str(e)}'}), 500
...
def get_voter_details(voter_id):
    try:
        ...
        votes = Vote.find_all({"voter_id": voter_id, "is_verified": True}, sort=[("vote_timestamp", -1)])
        voting_history = []
        for vote in votes:
        ...
        return jsonify({'success': False, 'message': 'Failed to load voter details'}), 500
...
def verify_voter(voter_id):
    try:
        ...
        update_data = {}
        if ver_type == 'all':
            update_data = {'email_verified': True, 'phone_verified': True, 'id_verified': True, 'face_verified': True, 'registration_status': 'verified', 'verified_at': datetime.utcnow(), 'verified_by': request.admin['admin_id']}
        ...
        _broadcast_voter_update('verify', {'voter_id': voter_id, 'verification_type': ver_type, 'full_name': voter['full_name']}, request.admin['admin_id'])
        log_admin_action(request.admin, "verify_voter", {"voter_id": voter_id, "verification_type": ver_type}, voter_id)
        return jsonify({'success': True, 'message': f'Voter {ver_type} verification completed', 'voter': updated})
    except Exception as e:
...
def update_voter_status(voter_id):
    try:
        ...
        if status not in VALID_VOTER_STATUSES:
            return jsonify({'success': False, 'message': f'Invalid status. Must be: {", ".join(VALID_VOTER_STATUSES)}'}), 400
        ...
        Voter.update_one({"voter_id": voter_id}, {"is_active": is_active, "updated_at": datetime.utcnow()})
        _broadcast_voter_update('status_update', {'voter_id': voter_id, 'full_name': voter['full_name'], 'new_status': status}, request.admin['admin_id'])
        log_admin_action(request.admin, "update_voter_status", {"voter_id": voter_id, "old_status": 'active' if voter.get('is_active') else 'inactive', "new_status": status}, voter_id)
        return jsonify({'success': True, 'message': f'Voter status updated to {status}'})
    except Exception as e:
...
def delete_voter(voter_id):
    try:
        ...
        if not voter:
            return jsonify({'success': False, 'message': 'Voter not found'}), 404
        Voter.update_one({"voter_id": voter_id}, {"is_active": False, "updated_at": datetime.utcnow()})
        _broadcast_voter_update('delete', {'voter_id': voter_id, 'full_name': voter['full_name']}, request.admin['admin_id'])
        log_admin_action(request.admin, "delete_voter", {"voter_id": voter_id, "voter_name": voter['full_name']}, voter_id)
...
def get_audit_logs():
    try:
        ...
        return jsonify({
            ...
            'logs': logs_data,
            'pagination': {'page': page, 'per_page': per_page, 'total': total, 'total_pages': total_pages},
            'filters': {'actions': actions, 'user_types': user_types, 'action': action, 'user_type': user_type}
        })
    except Exception as e:
...
def get_dashboard_reports():
    try:
        ...
        for i in range(7, -1, -1):
            ...
            count = Voter.count({"created_at": {"$gte": day_start, "$lt": day_end}, "is_active": True})
        ...
        for e in Election.find_all({"is_active": True}, limit=10):
            total_votes = Vote.count({"election_id": e['election_id'], "is_verified": True})
            constituency = e.get('constituency', 'General')
            constituency_voters = Voter.count({"constituency": constituency, "is_active": True})
            turnout = round(total_votes / constituency_voters * 100, 2) if constituency_voters else 0
        ...
        try:
            ...
            top_elections = [{"title": "National Election", "votes": 85000, "turnout": "75%"}]
        ...
        return jsonify({
            ...
            'reports': {
                ...
                'system_health': {"database_connections": "healthy", "api_response_time": "fast", "disk_usage": "85%", "memory_usage": "72%", "uptime": "99.8%"},
                'top_performing_elections': top_elections,
                'voter_growth': {
                    'this_month': Voter.count({"created_at": {"$gte": month_ago}, "is_active": True}),
                    'last_month': Voter.count({"created_at": {"$gte": month_ago - timedelta(days=30), "$lt": month_ago}, "is_active": True}),
        ...
        return jsonify({'success': False, 'message': f'Failed to load reports: {str(e)}'}), 500
...
def get_voter_analytics():
    try:
        ...
        verification_stats = {
            'fully_verified': Voter.count({"email_verified": True, "phone_verified": True, "id_verified": True, "face_verified": True, "is_active": True}),
            'partially_verified': Voter.count({"$or": [{"email_verified": False}, {"phone_verified": False}, {"id_verified": False}, {"face_verified": False}], "is_active": True}),
            'pending_verification': Voter.count({"registration_status": "pending", "is_active": True})
        }
        ...
        state_dist = list(Voter.get_collection().aggregate([
        ...
        active_last_30 = Voter.count({"last_login": {"$gte": thirty_days_ago}, "is_active": True})
        inactive = Voter.count({"last_login": {"$lt": thirty_days_ago}, "is_active": True})
        ...
        return jsonify({
            ...
            'analytics': {
                ...
                'state_distribution': [{"state": d["_id"], "count": d["count"]} for d in state_dist],
                'activity_level': {'active': active_last_30, 'inactive': inactive, 'total': active_last_30 + inactive},
                'total_voters': Voter.count({"is_active": True}),
                'generated_at': now.isoformat()
            }
        })
...
def export_report():
    try:
        ...
        if report_type == 'voters':
            ...
            for e in Election.find_all({"is_active": True}):
                export_data.append({
                    ...
                    'status': e.get('status'),
                    'voting_period': {'start': e.get('voting_start'), 'end': e.get('voting_end')},
        ...
        log_admin_action(request.admin, "export_report", {"report_type": report_type, "format": data.get('format', 'json'), "record_count": len(export_data)})
        return jsonify({'success': True, 'data': export_data, 'format': data.get('format', 'json'), 'count': len(export_data), 'exported_at': datetime.utcnow().isoformat()})
    except Exception as e:
...
def get_system_health():
    try:
        ...
        recent_errors = AuditLog.get_collection().find({"action": {"$regex": "error", "$options": "i"}}, limit=10).count()  # deprecated, use count_documents
        recent_errors = AuditLog.get_collection().count_documents({"action": {"$regex": "error", "$options": "i"}})
...
def update_system_settings():
    data = request.get_json()
    log_admin_action(request.admin, "update_system_settings", {"settings_updated": list(data.keys())})
...
def broadcast_message():
    try:
        ...
        log_admin_action(request.admin, "admin_broadcast", {"message": message, "type": broadcast_type, "recipients": "all_voters"})
