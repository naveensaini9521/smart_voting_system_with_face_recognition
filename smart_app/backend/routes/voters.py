...
def cast_vote():
    try:
        ...
        existing_vote = Vote.get_collection().find_one(
            {"voter_id": voter_id, "election_id": data["election_id"]}
        )
        ...
        Candidate.get_collection().update_one(
            {"candidate_id": data["candidate_id"]}, {"$inc": {"vote_count": 1}}
        )

        # Update election total votes
        Election.get_collection().update_one(
            {"election_id": data["election_id"]}, {"$inc": {"total_votes": 1}}
        )
...
def get_voting_history():
    try:
        ...
        votes = list(
            Vote.get_collection().find({"voter_id": voter_id}).sort("created_at", -1)
        )
            ...
            voting_history.append(
                {
                    ...
                    "election_id": vote["election_id"],
                    "election_title": (
                        election.get("title") if election else "Unknown Election"
                    ),
                    "candidate_name": (
                        candidate.get("full_name") if candidate else "Unknown Candidate"
                    ),
                    "candidate_party": (
                        candidate.get("party") if candidate else "Unknown Party"
                    ),
                    "voted_at": vote.get("created_at"),
        ...
        return (
            jsonify(
                {"voting_history": voting_history, "total_votes": len(voting_history)}
            ),
            200,
        )
...
def get_available_elections():
    try:
        ...
        for election in elections:
            ...
            if voter:
                existing_vote = Vote.get_collection().find_one(
                    {"voter_id": voter_id, "election_id": election["election_id"]}
                )
                has_voted = existing_vote is not None

            # Get candidates for this election
            candidates = list(
                Candidate.get_collection().find(
                    {
            ...
            election_data = {
                ...
                "has_voted": has_voted,
                "candidates": [
                    format_candidate_data(candidate) for candidate in candidates
                ],
        ...
        return (
            jsonify(
                {"elections": election_list, "total_elections": len(election_list)}
            ),
            200,
        )
...
def get_election_details(election_id):
    try:
        ...
        return jsonify({"election": election_data}), 200
            ...
            jsonify(
                {
                    ...
                    "message": (
                        "Eligible to vote" if is_eligible else "Not eligible to vote"
                    ),
                }
            ),
            200,
...
def generate_vote_hash(voter_id, election_id):
    ...
    unique_string = (
        f"{voter_id}{election_id}{datetime.utcnow().isoformat()}{uuid.uuid4()}"
    )
