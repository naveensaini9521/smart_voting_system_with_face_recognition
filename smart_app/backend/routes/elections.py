...
def get_elections_for_voter():
    ...
    try:
        ...
        return jsonify({"success": True, "elections_data": elections_data})

    except Exception as e:
        logger.error(f"Elections error: {str(e)}", exc_info=True)
        return (
            jsonify({"success": False, "message": "Failed to load elections data"}),
            500,
        )
    ...
    try:
        ...
        for election in all_elections:
            ...
            if voting_start is None or voting_end is None:
                logger.warning(
                    f"⚠️ Skipping election {election.get('title')}: missing or invalid dates"
                )
                continue
        ...
        for election in active_elections:
            try:
                ...
                election_data = {
                    ...
                    "require_face_verification": election.get(
                ...
                logger.error(
                    f"❌ Error processing election {election.get('election_id')}: {str(election_error)}"
                )
                continue
...
def get_upcoming_elections_enhanced():
    ...
    try:
            ...
            enhanced_elections.append(
                {
                    ...
                    "voting_start": (
                        election.get("voting_start").isoformat()
                        if election.get("voting_start")
                        else None
                    ),
                    "voting_end": (
                        election.get("voting_end").isoformat()
                        if election.get("voting_end")
                        else None
                    ),
                    "constituency": election.get(
                        "constituency", "General Constituency"
        ...
        return (
            jsonify({"success": False, "message": "Failed to load upcoming elections"}),
            500,
        )
...
def get_completed_elections():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to load completed elections"}
            ),
            500,
        )
...
def get_election_candidates(election_id):
    ...
    try:
        ...
        if not voting_start or not voting_end:
            return (
                jsonify(
                    {"success": False, "message": "Invalid election configuration"}
                ),
                400,
            )
        ...
        if current_time < voting_start:
            return (
                jsonify({"success": False, "message": "Voting has not started yet"}),
                400,
            )
        ...
        candidates = Candidate.find_all(
            {"election_id": election_id, "is_active": True, "is_approved": True}
        )
...
def cast_vote_in_election(election_id):
    ...
    try:
        ...
        logger.info(
            f"Vote casting attempt - Election: {election_id}, Voter: {voter['voter_id']}, Candidate: {candidate_id}"
        )

        if not candidate_id:
            logger.error("Candidate ID is missing from request")
            return (
                jsonify({"success": False, "message": "Candidate ID is required"}),
                400,
            )
        ...
        logger.info(
            f"⏰ Election time check - Current: {current_time}, Start: {voting_start}, End: {voting_end}"
        )
        logger.info(
            f"📊 Date types - Start: {type(voting_start)}, End: {type(voting_end)}"
        )

        # Check if election is active
        if not (voting_start and voting_end):
            logger.error(
                f"Invalid election configuration - missing voting dates: start={voting_start_raw}, end={voting_end_raw}"
            )
            return (
                jsonify(
                    {"success": False, "message": "Invalid election configuration"}
        ...
        if current_time < voting_start:
            ...
            logger.warning(
                f"Voting not started yet - Election: {election_id}, Starts in: {hours_remaining:.1f} hours"
            )
            return (
                jsonify(
                    {
        ...
        if current_time > voting_end:
            logger.warning(
                f"Voting ended - Election: {election_id}, Ended: {voting_end}"
            )
            return jsonify({"success": False, "message": "Voting has ended"}), 400
        ...
        if Vote.has_voted(election_id, voter["voter_id"]):
            logger.warning(
                f"Voter already voted - Election: {election_id}, Voter: {voter['voter_id']}"
            )
            return (
                jsonify(
                    {
        ...
        if not check_voter_eligibility(voter["voter_id"], election_id):
            logger.warning(
                f"Voter not eligible - Election: {election_id}, Voter: {voter['voter_id']}"
            )
            return (
                jsonify(
                    {
        ...
        if not candidate:
            logger.error(
                f"Invalid candidate: {candidate_id} for election: {election_id}"
            )
            return (
                jsonify(
                    {
        ...
        Candidate.get_collection().update_one(
            {"candidate_id": candidate_id}, {"$inc": {"vote_count": 1}}
        )

        # Update election total votes
        Election.get_collection().update_one(
            {"election_id": election_id}, {"$inc": {"total_votes": 1}}
        )
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to cast vote. Please try again."}
            ),
            500,
        )
...
def start_voting_session(election_id):
    ...
    try:
        ...
        print(
            f"📅 Date types - Start: {type(voting_start_raw)}, End: {type(voting_end_raw)}"
        )
        ...
        if has_voted:
            print(
                f"⚠️ Voter already voted - Election: {election_id}, Voter: {voter['voter_id']}"
            )
            return (
                jsonify(
                    {
        ...
        if not voter.get("is_active", False):
            ...
            return (
                jsonify({"success": False, "message": "Your account is not active"}),
                400,
            )
        ...
        session_id = (
            f"VS_{election_id}_{voter['voter_id']}_{int(datetime.utcnow().timestamp())}"
        )
        session_expires = datetime.utcnow() + timedelta(minutes=30)

        print(
            f"✅ Voting session ready - Session: {session_id}, Candidates: {len(candidates_data)}"
        )
        ...
        response_data = {
            ...
            "debug_info": {
                ...
                "voting_start_parsed": (
                    voting_start.isoformat() if voting_start else None
                ),
...
def get_election_results_for_voter(election_id):
    ...
    try:
        ...
        if (
            voting_end
            and current_time < voting_end
            and not election.get("results_published", False)
        ):
            return (
                jsonify(
                    {
        ...
        user_vote = None
        if has_voted:
            vote_record = Vote.find_by_election_and_voter(
        ...
        for result in results_data:
            if total_votes > 0:
                result["percentage"] = round(
                    (result["vote_count"] / total_votes) * 100, 2
                )
...
def get_active_elections(voter, election_type="all"):
    ...
    try:
        ...
        for election in elections:
            ...
            if not voting_start or not voting_end:
                logger.warning(
                    f"Election {election.get('election_id')} missing voting dates"
                )
                continue
            ...
            if not voting_start_dt or not voting_end_dt:
                logger.warning(
                    f"Election {election.get('election_id')} has invalid dates"
                )
                continue
