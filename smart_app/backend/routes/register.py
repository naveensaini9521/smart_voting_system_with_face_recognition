...
def send_voter_credentials(voter_data, voter_id, password):
    ...
    email_sent = send_email(
        voter_data["email"], "Voter Registration Successful", email_body
    )
...
def send_otp_registration():
    ...
    try:
            ...
            return jsonify(
                {
                    ...
                    "message": "OTP sent successfully (development mode)",
                    "debug_otp": mock_otp,
                    "channels": {"email_sent": bool(email), "sms_sent": bool(phone)},
                }
            )
        ...
        if email:
            ...
            <body>
                ...
                <div style="background-color: #f8f9fa; 
                    padding: 15px; border-radius: 5px; text-align: center; margin: 10px 0;">
        ...
        response_data = {
            ...
            "channels": {"email_sent": email_sent, "sms_sent": sms_sent},
        }
...
def verify_otp_registration():
    ...
    try:
        ...
        is_valid = OTP.verify_otp(
            email=email, phone=phone, otp_code=otp_code, purpose=purpose
        )
        ...
        if is_valid:
            ...
            if otp_code and len(otp_code) == 6 and otp_code.isdigit():
                ...
                return (
                    jsonify({"success": False, "message": "Invalid or expired OTP"}),
                    400,
                )
...
def register_voter():
    ...
    try:
        data = request.get_json()
        logger.info(
            f"Registration data received for: {data.get('full_name', 'Unknown')}"
        )
        ...
        if Voter.find_by_national_id(data["national_id_number"]):
            return (
                jsonify(
                    {"success": False, "message": "National ID already registered"}
                ),
                400,
            )
        ...
        if not voter_doc or "voter_id" not in voter_doc:
            logger.error(
                f"Failed to retrieve voter document after creation: {mongo_id}"
            )
            return (
                jsonify(
                    {
        ...
        logger.info(
            f"New voter registered successfully. Voter ID: {actual_voter_id}, MongoDB ID: {mongo_id}"
        )
...
def complete_registration(voter_id):
    ...
    try:
        ...
        if not password:
            ...
            password = (
                voter.get("date_of_birth", "").strftime("%Y%m%d")
        ...
        Voter.update_one(
            {"voter_id": voter_id}, {"$set": {"registration_status": "completed"}}
        )
        ...
        response_data = {
            ...
            "credentials_sent": {"email": email_sent, "sms": sms_sent},
            "voter_id": voter_id,
            "password": password,
        }
        ...
        if not email_sent or not sms_sent:
            response_data["warning"] = (
                "Credentials could not be sent via all channels. Please contact support."
            )
        ...
        return (
            jsonify({"success": False, "message": "Registration completion failed"}),
            500,
        )
    ...
    try:
        ...
        if not result.is_match:
            ...
            if error_details.get("duplicate_detected"):
                ...
                if existing_voter:
                    error_message["existing_voter_name"] = existing_voter.get(
                        "full_name", "Unknown"
                    )
                ...
                return jsonify({"success": False, **error_message}), 409
        ...
        if (
            voter.get("email_verified")
            and voter.get("phone_verified")
            and voter.get("id_verified")
        ):
            update_data["registration_status"] = "completed"

        # Update voter in database
        update_result = Voter.update_one({"voter_id": voter_id}, {"$set": update_data})
        ...
        response_data = {
            ...
            "registration_completed": updated_voter.get("registration_status")
            == "completed",
            "voter_data": {
                "voter_id": voter_id,
        ...
        return (
            jsonify(
                {"success": False, "message": f"Face registration failed: {str(e)}"}
            ),
            500,
        )
    ...
    try:
        ...
        if voter.get("face_verified"):
            return (
                ...
                200,
            )  # Return 200 with success=false to indicate check result
        ...
        return (
            jsonify({"success": False, "message": "Face duplicate check failed"}),
            500,
        )
...
def check_voter(voter_id):
    ...
    try:
        ...
        return jsonify({"success": False, "message": "Failed to fetch voter data"}), 500
...
def knn_status():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "model_status": (
                    "loaded" if knn_face_service.knn_model else "not_loaded"
                ),
                "statistics": stats,
        ...
        return jsonify({"success": False, "message": "Failed to get KNN status"}), 500
...
def knn_reindex():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": f"Failed to reindex KNN: {str(e)}"}),
            500,
        )
...
def face_system_stats():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Failed to get system statistics"}),
            500,
        )
...
def find_similar_faces():
    ...
    try:
                    ...
                    results.append(
                        {
                            ...
                            "confidence": (
                                ...
                                else "MEDIUM" if match["similarity"] > 0.7 else "LOW"
                            ),
                        }
                    )
        ...
        return (
            jsonify({"success": False, "message": "Failed to find similar faces"}),
            500,
        )
