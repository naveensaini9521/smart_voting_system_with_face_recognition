...
def login():
    ...
    try:
        ...
        if not voter_id or not password:
            return (
                jsonify(
                    {"success": False, "message": "Voter ID and password are required"}
                ),
                400,
            )
        ...
        if not voter:
            return (
                jsonify({"success": False, "message": "Invalid Voter ID or password"}),
                401,
            )
        ...
        limited_voter_data = {"voter_id": voter["voter_id"], "email": voter["email"]}
        temp_token = generate_token(limited_voter_data)

        # Update last login
        try:
            Voter.update_one(
                {"voter_id": voter_id}, {"$set": {"last_login": datetime.utcnow()}}
            )
        except Exception as e:
    ...
    try:
        ...
        if result.is_match:
            ...
            try:
                ...
                logger.info(
                    f"Updated last_face_verification and last_login for voter: {voter_id}"
                )
            except Exception as e:
            ...
            logger.warning(
                f"⚠️ Debug mode: Generating token despite failed verification: {final_token[:50]}..."
            )
...
def admin_login():
    ...
    try:
        ...
        if not admin:
            return (
                jsonify({"success": False, "message": "Invalid admin credentials"}),
                401,
            )
        ...
        if not admin.get("is_active", True):
            return (
                jsonify(
                    {"success": False, "message": "Admin account has been deactivated"}
                ),
                401,
            )
...
def check_auth():
    ...
    if user_type == "voter":
        ...
        if not voter:
            return jsonify({"success": False, "message": "Voter not found"}), 401
        ...
        return jsonify({"success": True, "user_type": "voter", "user_data": voter_data})
        ...
        return jsonify({"success": True, "user_type": "admin", "user_data": admin_data})

    else:
        return jsonify({"success": False, "message": "Unknown user type"}), 401
...
def debug_voters():
    ...
    try:
            ...
            voter_list.append(
                {
                    ...
                    "date_of_birth": (
                        str(voter.get("date_of_birth"))
                        if voter.get("date_of_birth")
                        else None
                    ),
                    "has_password": "password_hash" in voter
                    and voter["password_hash"] is not None,
                    "password_hash_length": (
                        len(voter["password_hash"])
                        if "password_hash" in voter and voter["password_hash"]
                        else 0
                    ),
                }
            )
        ...
        return jsonify(
            {"success": True, "total_voters": len(voter_list), "voters": voter_list}
        )
...
def debug_admins():
    ...
    try:
            ...
            admin_list.append(
                {
                    ...
                    "has_password": "password_hash" in admin
                    and admin["password_hash"] is not None,
                    "password_hash_length": (
                        len(admin["password_hash"])
                        if "password_hash" in admin and admin["password_hash"]
                        else 0
                    ),
        ...
        return jsonify(
            {"success": True, "total_admins": len(admin_list), "admins": admin_list}
        )
...
def debug_test_token(voter_id):
    ...
    try:
        ...
        if not voter:
            return (
                jsonify({"success": False, "message": f"Voter {voter_id} not found"}),
                404,
            )
        ...
        return jsonify(
            {
                ...
                "face_encoding_type": (
                    face_encoding.get("encoding_type") if face_encoding else None
                ),
                "registration_complete": voter.get("registration_status")
                == "completed",
            }
        )
