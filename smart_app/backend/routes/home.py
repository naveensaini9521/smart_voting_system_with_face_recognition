...
def get_stats():
    try:
        ...
        active_elections = Election.get_collection().count_documents(
            {"status": "active"}
        )
        completed_elections = Election.get_collection().count_documents(
            {"status": "completed"}
        )
        ...
        face_success = mongo.db.voter_face_encodings.count_documents(
            {"is_active": True}
        )
...
def get_process_steps():
    ...
    return jsonify(
        {"success": True, "process_steps": steps, "total_time": "Under 5 minutes"}
    )
    ...
    return jsonify(
        {
            ...
            "health": {
                ...
                "uptime": "99.9%",
                "database_size_mb": (
                    round(db_stats.get("dataSize", 0) / (1024 * 1024), 2)
